...
instance: has_sub mynat := ⟨aux_sub_nat_nat⟩
...
@[simp] theorem aux_minus_succ_succ : succ m - succ n = m - n := rfl
...
begin
  ...
  rwa [add_succ, aux_minus_succ_succ],
end
...
begin
  induction m with m hm,
    repeat { rw zero_sub <|> rw zz },
  rw succ_sub_one,
  -- How do we use the inductive hypothesis?
  -- Do we need to use le_total_order?
  sorry,
end
...
begin
  ...
  rw [aux_minus_succ_succ, ←add_one_succ, sub_switch_one, add_sub_one],
end
